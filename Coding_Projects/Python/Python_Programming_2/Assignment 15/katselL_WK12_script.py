...
class classNLTKQuery:
    ...
    def textCorpusInit(self, thePath):
        ...
        try:
            ...
            self.VocSize = nltk.FreqDist(self.tokens)
        except:
    ...
    def printVocabSize(self):
        ...
        print ("\n\nVocabulary Size: ", '{:,}'.format(len(self.VocSize)))
    ...
    def searchWordOccurrence(self):
        ...
        if myWord:
            ...
            myWordUpper = myWord.upper()
            found = 0
            for word, freq in self.VocSize.items():
                if myWordUpper == word.upper():
                    print("\n\n", myWord, "Occurs", freq, "Times")
                    found = 1
                    break
            if found == 0 :
                print("\n\n", myWord, "Does not 0ccur")
    ...
    def generateConcordance(self):
        ...
        if myWord:
            ...
            conList = self.TextCorpus.concordance_list(myWord)  
            if len(conList) == 0:
                print(myWord, "\n\nNot in Text")
            i = 0
            while i < 100 and i < len(conList):
                print(conList[i].line)
                i += 1
    ...
    def generateSimiliarities(self):
        ...
        if myWord:
            ...
            simList = self.TextCorpus.similar(myWord)
    ...
    def printWordIndex(self):
        ...
        if myWord:
            ...
            myWordUpper = myWord.upper()
            word_index = None
            for index, (word, freq) in enumerate(self.VocSize.items()):
                if myWordUpper == word.upper():
                    word_index = index
                    break
            if word_index is not None:
                print("\n\n", myWord, "First Occurs at Index:", word_index)
            else:
                print("\n\n", myWord, "Does not Occur")

    def printVocabulary(self):
        print ("\n\nCompiling Vocabulary Frequencies")
              
        vocab_freq = nltk.FreqDist(self.TextCorpus)  # Compute frequency distribution
