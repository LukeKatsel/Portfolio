...
int main(int argc , char *argv[])
{
    ...
    while (1){                                                                  // infinite loop to receive one client after another
        ...
        while (1){                                                              // infinite loop for non-blocking
            ...
            else if (bytesRead == 0)
            {
                ...
                fflush(stdout);
                break;                                                          // break statement to jump out of loop when client disconnects
            }
            else if (errno == EAGAIN || errno == EWOULDBLOCK){                  // Error checking for non-blocking
